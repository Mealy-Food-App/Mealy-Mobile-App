...
export default ResetPasswordScreen = () => {
  ...
  const validButton = {
    ...
    fontSize: 14,
    borderColor: 'rgba(230, 159, 20, 0.5)',
      ...
      <>
        ...
        <Formik
          ...
          validationSchema={yup.object().shape({
            email: yup.string().email('Email address is invalid').required('Email address required.'),
            password: yup
              .string()
        ...
        >
          {({ values, handleChange, setFieldValue, errors, setFieldTouched, touched, isValid, handleSubmit }) => (
            <View style={styles.formContainer}>
              ...
              {touched.email && errors.email && <Text style={styles.formerror}>{errors.email}</Text>}
              ...
              <View style={userInput}>
                ...
                <TouchableOpacity style={{ position: 'absolute', top: 12, left: 276, height: 40 }} onPressIn={managePasswordVisiblity}>
                  <Image
                    source={passwordVisibility ? require('../assets/icons/openeye.png') : require('../assets/icons/eye-off.png')}
                    style={{ width: 18, height: 18, tintColor: COLORS.primary }}
                  />
                </TouchableOpacity>
              </View>

              {touched.password && errors.password && <Text style={styles.formerror}>{errors.password}</Text>}
